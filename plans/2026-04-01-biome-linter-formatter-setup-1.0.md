...
- [ ] **Task 3.1: 創建 .vscode/settings.json**
  - 設置 Biome 為預設 formatter
  - 啟用 format on save
  - 設置 TypeScript/JavaScript/TSX/JSX 使用 Biome
  - 禁用其他 formatter（避免衝突）
  
  **理由：** 確保編輯器自動格式化，提升開發體驗

- [ ] **Task 3.2: 創建 .vscode/extensions.json**
  - 推薦安裝 Biome 擴充功能
  - 推薦其他相關擴充功能（可選）
  
  **理由：** 提示團隊成員安裝必要的編輯器擴充功能

### 階段四：Git Hooks（可選但建議）

- [ ] **Task 4.1: 安裝 simple-git-hooks**
  - 使用 pnpm 安裝 simple-git-hooks（比 husky 更輕量）
  - 或選擇使用 Husky + lint-staged
  
  **理由：** 在 commit 前自動執行 lint，防止不合格程式碼進入代碼庫

- [ ] **Task 4.2: 配置 pre-commit hook**
  - 設置對 staged 檔案執行 biome check --fix
  - 只檢查暫存的檔案（提升效能）
  
  **理由：** 自動修正提交的程式碼，確保代碼庫品質

- [ ] **Task 4.3: 添加 lint-staged 配置（如果使用 Husky）**
...
2. **優先執行：** 階段三（Editor integration）- 提升開發體驗
3. **建議執行：** 階段四（Git hooks）- 確保代碼品質
4. **最後執行：** 階段五（驗證）- 確認一切正常
...
3. `.vscode/settings.json` - 新建
4. `.vscode/extensions.json` - 新建
5. `package.json` - 添加 simple-git-hooks（可選）
