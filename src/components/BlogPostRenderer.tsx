...
export default function BlogPostRenderer({ data, query, variables, relatedPosts = [] }: BlogPostRendererProps) {
  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--warm-beige))]">
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-[hsl(var(--muted-bronze))]/10">
        <div className="px-6 sm:px-8 lg:px-16 xl:px-24">
          <div className="flex items-center justify-between h-16">
            <a
              href="/blog"
              className="flex items-center gap-2 text-[hsl(var(--deep-espresso))] hover:text-[hsl(var(--muted-bronze))] transition-colors"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
              </svg>
              <span className="text-sm">返回日誌</span>
            </a>
          </div>
        </div>
      </nav>
      ...
      <section className="relative">
        <div data-tina-field={tinaField(blog, "heroImage")} className="aspect-[21/9] lg:aspect-[3/1] overflow-hidden">
          {blog.heroImage && (
            <img
              src={blog.heroImage}
              alt={blogTitle}
      ...
      <article className="relative -mt-20 lg:-mt-32 z-10">
        <div className="px-6 sm:px-8 lg:px-16 xl:px-24">
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl shadow-lg p-8 lg:p-12 mb-8">
              <div className="flex flex-wrap items-center gap-4 mb-6">
                {blog.category && (
                  <span data-tina-field={tinaField(blog, "category")} className="px-4 py-1.5 bg-[hsl(var(--blush-mist))]/10 rounded-full text-sm text-[hsl(var(--blush-mist))]">
                    {blog.category}
                  </span>
                )}
                <div className="flex items-center gap-4 text-sm text-[hsl(var(--deep-espresso))]/60">
                  {blog.pubDate && (
                    <span className="flex items-center gap-1.5" data-tina-field={tinaField(blog, "pubDate")}>
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-6 8h8m-8 4h8m-4-4v4m0 0v4m0-4h4m-4 0H8m8 0h4m-4 0v4m0-4V8m0 8v4m0-4h4m-4 0H8"></path>
                      </svg>
                      <FormattedDate date={blog.pubDate} />
                    </span>
                  )}
                  {blog.readTime && (
                    <span data-tina-field={tinaField(blog, "readTime")} className="flex items-center gap-1.5">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <h1 data-tina-field={tinaField(blog, "title")} className="text-3xl lg:text-4xl font-medium text-[hsl(var(--deep-espresso))] mb-4 leading-tight">
                {blogTitle}
              </h1>

              {blog.subtitle && (
                <p data-tina-field={tinaField(blog, "subtitle")} className="text-lg text-[hsl(var(--muted-bronze))] mb-6">
                  {blog.subtitle}
                </p>
              )}
              ...
              <div className="flex items-center gap-4 pt-6 border-t border-[hsl(var(--muted-bronze))]/10">
                ...
                <div>
                  <p data-tina-field={tinaField(blog, "author")} className="font-medium text-[hsl(var(--deep-espresso)]">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-8 lg:p-12 mb-12">
              <div data-tina-field={tinaField(blog, "body")} className="prose prose-lg max-w-none prose-headings:text-[hsl(var(--deep-espresso))] prose-p:text-[hsl(var(--deep-espresso))]/80 prose-strong:text-[hsl(var(--deep-espresso))] prose-li:text-[hsl(var(--deep-espresso))]/80 prose-a:text-[hsl(var(--muted-bronze))] prose-a:no-underline hover:prose-a:underline">
                ...
                <div className="mt-12 pt-8 border-t border-[hsl(var(--muted-bronze))]/10">
                  <div data-tina-field={tinaField(blog, "tags")} className="flex flex-wrap gap-2">
                    {blog.tags.filter(Boolean).map((tag) => (
                      <span key={tag} className="px-4 py-2 bg-[hsl(var(--warm-beige))] rounded-full text-sm text-[hsl(var(--muted-bronze))]">
                        {tag}
                      </span>
                    ))}
                  </div>
